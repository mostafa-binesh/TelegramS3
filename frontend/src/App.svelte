<script lang="ts">
  ...
  let shareError = '';
  let shareBusy = false;
  let showSharedLinksModal = false;
  let sharedLinksTarget: ObjectEntry | null = null;
  ...
  async function refreshTelegramSettings() {
    try {
      const [response, storageResponse] = await Promise.all([getTelegramSettings(), getStorageSettings()]);
      applyTelegramSettings(response.settings);
      applyStorageSettings(storageResponse);
    } catch (cause) {
  ...
  function applyTelegramSettings(settings: TelegramSettings) {
    ...
    telegramAccountPhone = settings.telegram_account_phone ?? '';
  ...
  async function saveStorageSettingsForm() {
    storageSettingsBusy = true;
    storageSettingsError = '';
    ...
    try {
      ...
      storageSettingsBusy = false;
    }
  }

  async function removeCurrentConnection(phoneConfirmation: string, deleteUploadedFiles: boolean) {
    if (!session?.csrf_token) throw new Error('Your session has expired. Sign in again.');
    telegramSettingsBusy = true;
    try {
      const result = await removeTelegramConnection(session.csrf_token, deleteUploadedFiles, phoneConfirmation);
      buckets = [];
      listing = null;
      selectedKeys = [];
      telegramApiId = '';
  ...
  async function createShareFromModal() {
    ...
    shareError = '';
    try {
      const result = await createShareLink(session?.csrf_token, selectedBucket, shareTarget.key, trimmed ? Number(trimmed) : null, shareDescription.trim());
  ...
  async function openSharedLinksModal(obj: ObjectEntry) {
    await loadAdminModals();
    sharedLinksTarget = obj;
    sharedLinks = [];
    sharedLinksError = '';
    showSharedLinksModal = true;
    await refreshSharedLinks();
  }

  async function refreshSharedLinks() {
    if (!sharedLinksTarget) return;
    sharedLinksBusy = true;
    sharedLinksError = '';
    try {
      const result = await listShareLinks(session?.csrf_token, selectedBucket, sharedLinksTarget.key);
      sharedLinks = result.links;
    } catch (cause) {
      sharedLinksError = normalizeError(cause);
    } finally {
      sharedLinksBusy = false;
    }
  }

  async function changeSharedLinkExpiry(id: string, seconds: number | null) {
    sharedLinksBusy = true;
    sharedLinksError = '';
    try {
      await updateShareLinkExpiry(session?.csrf_token, id, seconds);
      notifySuccess(seconds === null ? 'Link set to follow the file expiry.' : 'Link expiry updated.');
      await refreshSharedLinks();
    } catch (cause) {
      sharedLinksError = normalizeError(cause);
    } finally {
  ...
  async function revokeSharedLink(id: string) {
    sharedLinksBusy = true;
    sharedLinksError = '';
    try {
      await revokeShareLink(session?.csrf_token, id);
      notifySuccess('Shared link revoked.');
      await refreshSharedLinks();
      await refreshObjects();
    } catch (cause) {
      sharedLinksError = normalizeError(cause);
  ...
  function closeSharedLinksModal() {
    if (sharedLinksBusy) return;
    showSharedLinksModal = false;
    sharedLinksTarget = null;
    sharedLinks = [];
    sharedLinksError = '';
  }
...
<style>
  :global {
  .shell.signed-in{width:calc(100% - 230px);max-width:none;margin:0 0 0 230px;padding:clamp(20px,3vw,40px);min-height:100vh}
  ...
  @media(max-width:900px){.shell.signed-in{width:100%;margin-left:0;padding:0 clamp(12px,3vw,24px) 24px}.console-header{margin-top:clamp(18px,4vw,28px);align-items:flex-start;flex-direction:column}.settings-summary{grid-template-columns:1fr}}
  ...
  @media(max-width:620px){
    ...
    .section-head .toolbar-actions{width:100%;display:grid}
    .section-head .toolbar-actions button{width:100%}
