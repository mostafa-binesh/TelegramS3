...
<section class="card surface">
  ...
  <div class="table-scroll"><table><thead><tr><th>Object</th><th>Progress</th><th>Status</th><th>Actions</th></tr></thead><tbody>
    {#each visible as job (job.id)}<tr><td><strong>{job.key}</strong><small>{job.bucket} · {new Date(job.created_at*1000).toLocaleString()}</small><small>{job.id}</small></td>
      ...
      <td><span class="state" class:done={['completed','cleaned'].includes(job.state)}>{job.state==='cleaned'?'completed':job.state.replaceAll('_',' ')}</span>{#if job.error}<small class="error-hint">{job.error}</small>{/if}{#if job.state==='retry_wait'}<small>Retry {new Date(job.next_retry*1000).toLocaleTimeString()} · attempt {job.attempts}</small>{/if}</td>
