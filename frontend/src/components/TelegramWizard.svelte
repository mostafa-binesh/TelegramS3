...
<style>
  ...
  @media(max-width:560px){.wizard-topbar{align-items:flex-start}.close-wizard span{display:none}.form-grid{grid-template-columns:1fr}.wizard-step-card{padding:20px}.step-heading{gap:11px}.large-step-number{width:42px;height:42px;border-radius:13px}.step-heading h3{font-size:1.3rem}.wizard-rail ol{overflow:auto}.wizard-rail li{min-width:74px}.wizard-rail li small{display:none}.wizard-footer{position:sticky;bottom:0;padding-top:12px;background:linear-gradient(180deg,transparent,#f4f8fc 28%)} }
</style>
