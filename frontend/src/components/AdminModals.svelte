...
{#if showMove}<div class="modal-backdrop" role="presentation" on:click={(event) => event.target === event.currentTarget && (showMove = false)}><form class="modal-card compact-modal" on:submit|preventDefault={onMove}><div class="section-head"><div><p class="card-label">Move selected items</p><h2>Choose destination</h2></div><button class="icon-button" type="button" on:click={() => showMove = false}>×</button></div><label><span>Destination bucket</span><select bind:value={moveBucket}>{#each buckets as bucket (bucket.name)}<option value={bucket.name}>{bucket.name}</option>{/each}</select></label><label><span>Destination folder</span><input bind:value={movePrefix} placeholder="optional/folder/" /></label><p class="fine-print">Files are copied, then removed from the current bucket.</p><button class="primary" type="submit" disabled={busy || !moveBucket}>Move files</button></form></div>{/if}
