...
<aside>
  ...
  <footer><strong>{username}</strong><button class="sign-out" type="button" on:click={onLogout} disabled={busy}>{busy ? 'Signing out…' : 'Sign out'}</button></footer>
</aside>
<style>
  aside{position:fixed;inset:0 auto 0 0;width:230px;background:#142235;color:#e8eef6;padding:30px 18px;display:flex;flex-direction:column;z-index:5}.brand{display:flex;gap:12px;align-items:center;font-size:28px;color:white;text-decoration:none;padding:0 12px 34px}.brand>span:last-child{font-size:16px;font-weight:650}.brand small{display:block;font-size:11px;color:#9aacbf;margin-top:4px;font-weight:400}.brand-mark{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:#2d78bd;color:#fff;font-weight:800}nav{display:grid;gap:5px}nav button{width:100%;min-width:0;text-align:left;background:transparent;color:#c8d3e0;border-radius:8px;padding:12px 15px;font-size:14px;justify-content:flex-start}.nav-icon{width:22px;height:22px;display:grid;place-items:center;line-height:1}.nav-icon svg{width:18px;height:18px;display:block}.chosen{background:#29425e;color:white}footer{margin-top:auto;display:grid;gap:10px;font-size:13px;padding:14px}footer button{padding:8px 0;justify-content:flex-start}.sign-out{background:transparent;color:#ffd2d2;border:1px solid rgba(255,210,210,.28);justify-content:center}.sign-out:hover:not(:disabled){background:rgba(179,56,56,.22);border-color:#e38c8c} @media(max-width:900px){aside{position:static;width:100%;padding:14px clamp(12px,3vw,24px)}.brand{padding:0 0 14px}.brand>span:last-child{font-size:15px}nav{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px}nav button{justify-content:center;text-align:center;flex-direction:column;padding:10px 4px;font-size:12px;line-height:1.2}.nav-icon{font-size:16px}footer{display:flex;justify-content:space-between;align-items:center;padding:12px 0 0}}@media(max-width:380px){.brand{gap:8px}.brand-mark{width:32px;height:32px}.brand>span:last-child{font-size:14px}.brand small{font-size:10px}nav button{font-size:11px;padding-right:2px;padding-left:2px}}
</style>
