...
<section class="card surface">
  <div class="section-head"><div class="heading-row">{#if selectedBucket}<button class="icon-button back-button" type="button" title="Back to parent" aria-label="Back to parent folder" on:click={onBack}>←</button>{/if}<div><p class="card-label">Buckets and files</p><h2>{selectedBucket ? `Bucket / ${selectedBucket}` : 'Your buckets'}</h2></div></div>
  ...
  {#if !selectedBucket}
    ...
    {:else}<ul class="checks">{#each buckets as bucket (bucket.name)}<li><div class="bucket-row"><button type="button" class="btn-link" on:click={() => onOpenBucket(bucket.name)}>{bucket.name}<small>created {formatTimestamp(bucket.created_at)}</small></button><button type="button" class="danger-button compact-action" on:click={() => onRemoveBucket(bucket.name)} disabled={busy}>Delete</button></div></li>{/each}</ul>{/if}
  {:else}
